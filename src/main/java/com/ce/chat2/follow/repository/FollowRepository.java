...
public interface FollowRepository extends JpaRepository<Follow, Integer> {
    ...
    // 현재 친구 목록
    @Query("SELECT f FROM Follow f JOIN FETCH f.to WHERE f.from = :from AND f.isBreak = false")
    ...
    @Query("SELECT f FROM Follow f JOIN FETCH f.to u WHERE f.from = :from AND u.name LIKE :name AND f.isBreak = false")
